// ─────────────────────────────────────────────────────
// Multi-tenant tool runtime. Reuses the exact 14 tool handlers from the local
// server, but api()/load()/save()/dates resolve PER-USER via AsyncLocalStorage,
// so every request runs isolated in its own user's context.
// ─────────────────────────────────────────────────────
...
const ctx = () => {
  ...
  if (!c) throw new Error("no user context");
  return c;
};

// Per-user resolvers the handlers call by their bare names:
const api  = (path, params) => ctx().api(path, params);
const load = () => ctx().load();
const save = (data) => ctx().save(data);

// Per-user timezone-aware dates (host serves many users in different zones).
...
function dateAgo(nDays) { const d = new Date(); d.setDate(d.getDate() - nDays); return localDate(d); }
function isoToLocalDate(iso) { return iso ? localDate(new Date(iso)) : null; }
function prevLocalDate(dateStr) {
  if (!dateStr) return null;
  const d = new Date(`${dateStr}T12:00:00Z`); d.setDate(d.getDate() - 1);
  const [y, m, day] = dateStr.split("-").map(Number);
  const base = new Date(Date.UTC(y, m - 1, day, 12)); base.setUTCDate(base.getUTCDate() - 1);
...
// Public entry: run a tool in a user's context.
export async function runTool(context, name, args = {}) {
  return als.run(context, () => handleTool(name, args));
}

// ── PREDEFINED PROMPTS (MCP prompts capability) ──────
const DASHBOARD_BRIEF = `
Respond DIRECTLY in the chat as a clean, glanceable card in words — no artifact, HTML, or download. Shape:
**<one-line verdict, bold>**
<emoji 🟢/🟡/🔴> Recovery <n>% · <zone>
HRV <n>ms (<+/- vs base>) · RHR <n> · Sleep <Xh Ym> (deep <Xh Ym>) · Strain <n> yesterday
<if food/notes: ONE pattern line, not proof>
**→ Today:** <one specific action>
Tight, not paragraphs. Only real tool values; if recovery unscored or no food, say so in one line — never fabricate
numbers or a protein→HRV constant. Build a visual artifact only if the user explicitly asks for the dashboard.`;
...
or output any code — render inline in the chat with the Visualizer only.

STEP 1 — Fetch from my Whoop MCP tools: latest recovery (score %, HRV, resting HR); latest sleep (total, in-bed, bedtime,
deep/SWS min, sleep need); RHR + HRV for the last 14 days (sparklines); all runs in the last 14 days (date, distance,
duration, avg HR, max HR, HR-zone durations); today's strain + calories; and get_nutrition for today (calories/protein
totals — note if estimated — and water intake in L + % of target).

STEP 2 — Compute from run data only: pace per km (duration ÷ distance, m'ss"); longest run (km); fastest pace; best pace
among runs with avg HR ≤ 143. Do NOT invent/estimate. If a field isn't returned, show "—" in that slot.

STEP 3 — Race & goals ONLY if I mentioned them earlier in THIS chat: if named, show a race header (name + date + days
remaining from today) and goal cards (target, progress bar from the relevant metric, "current" sub). If NOT mentioned,
OMIT the race header and goals entirely — never invent a race, date, or goals. Steps aren't in Whoop — "—" unless I give a number.

LAYOUT (mobile width, dark-mode safe, max 2 cards per row):
1. Header: today's date + live clock (race name/date + days-remaining only if provided).
2. Three rings: recovery %, sleep as hours-of-need, and WATER of target (real, from get_nutrition) — subs: HRV/RHR, bedtime/deep, litres so far. Whoop is MEASURED; food calories may be photo ESTIMATES — mark those with "~".
3. Goals cards — only if goals were provided; else skip.
4. Engine: RHR and HRV stacked rows — value, 14-day sparkline, delta vs 14-day baseline. Skip VO2 max / HR recovery unless returned.
5. Running: strip of longest / fastest / best@≤143, then a line chart of pace per run with avg HR under each point, dates on x-axis.
After rendering, list in one short line which fields came from Whoop and which were "—".
`;

export const PROMPTS = [
  { name: "dashboard", description: "Your Whoop training dashboard as an inline visual — recovery, sleep, RHR/HRV trends, running. Mention a race or goals in chat first for goal progress." },
  { name: "morning_read", description: "Your morning read — recovery, sleep & strain as a visual brief with one action." },
  { name: "evening_wrap", description: "End-of-day wrap-up — how today went vs targets, and what to do tonight." },
  { name: "week_review",  description: "Your weekly review — the week's arc and one thing to improve." },
];
const PROMPT_MAP = {
  morning_read: "Call get_daily_brief, then build my morning read.",
  evening_wrap: "Call get_day_report, then build my end-of-day wrap-up.",
  week_review:  "Call get_week_review, then build my weekly review with the arc and standout days.",
};
export function getPrompt(name) {
  if (name === "dashboard") return { description: "Render my training dashboard as an inline visual", messages: [{ role: "user", content: { type: "text", text: TRAINING_DASHBOARD } }] };
  const lead = PROMPT_MAP[name];
  if (!lead) throw new Error(`Unknown prompt: ${name}`);
  return { description: "Render as a visual dashboard", messages: [{ role: "user", content: { type: "text", text: `${lead}\n${DASHBOARD_BRIEF}` } }] };
}
