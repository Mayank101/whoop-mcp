...
Render as a single self-contained HTML artifact — a health dashboard in the "MODERNIST" design system (flat, architectural).
Font: Archivo (Google Fonts) 400/600/800. Tokens: bg #f3f2f2, surface #eae9e9, text #201e1d, accent #ec3013 (red),
divider = ink @40%. Rules: ZERO border radius; structure drawn with 2px section rules + 1px inner rules, nothing floats;
modular grid, everything flush-left; UPPERCASE muted micro-labels (.1em); numbers Archivo-800 tight; accent used sparingly
(hero number, primary button, active nav). Recovery = a GIANT ~120px accent number + zone word + a horizontal two-segment
proportion bar (NOT a ring). Layout: top bar "VITAL." + nav + date + solid-accent action; verdict panel (giant recovery
number, proportion bar, plain-language verdict, HRV/RHR/resp metric row); then 2px-ruled sections for sleep, strain, fuel,
notes; a themed week table; one accent-marked action; small footer "patterns to notice, not medical advice; recovery is
driven mainly by HRV, resting heart rate and sleep." Number → meaning: the read leads, numbers are evidence beside it.
Use only real tool values; never invent numbers or a protein→HRV constant; if data is missing say so and invite logging.
Inline all CSS (no external files); responsive; respect reduced motion.`;
