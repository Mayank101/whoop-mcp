...
export async function initDb() {
  ...
  await q(`CREATE TABLE IF NOT EXISTS user_data (
    user_id INTEGER PRIMARY KEY REFERENCES users(id) ON DELETE CASCADE,
    targets_json TEXT, food_json TEXT, notes_json TEXT, water_json TEXT)`);
  await q(`ALTER TABLE user_data ADD COLUMN IF NOT EXISTS water_json TEXT`).catch(()=>{});
...
export async function createUser(email, passwordHash) {
  ...
  const user = rows[0];
  await q("INSERT INTO user_data (user_id, targets_json, food_json, notes_json, water_json) VALUES ($1,$2,$3,$4,$5)",
    [user.id, "{}", "{}", "{}", "{}"]);
  return user;
}
...
export async function getUserData(userId) {
  ...
  return {
    ...
    water: JSON.parse(r.water_json || "{}"),
  };
}
export async function saveUserData(userId, data) {
  await q(`INSERT INTO user_data (user_id, targets_json, food_json, notes_json, water_json) VALUES ($1,$2,$3,$4,$5)
    ON CONFLICT (user_id) DO UPDATE SET targets_json=$2, food_json=$3, notes_json=$4, water_json=$5`,
    [userId, JSON.stringify(data.targets || {}), JSON.stringify(data.food_history || {}), JSON.stringify(data.notes || {}), JSON.stringify(data.water || {})]);
}
